...
const createStoryStep = (text, options, image = null) => ({
  text,
  options: options.map(opt => createOption(opt.text, opt.next)), // Corrected to `options`
  image,
});
...
const story = [
  ...
  createStoryStep(
      ...
      [
        ...
        {text: 'Head out to Bloodstone Island on your own.', next: 3}, // Corrected missing period
      ],
      IMAGE_PATHS.map,
  ),
  createStoryStep(
  ...
  createStoryStep(
      ...
      [
        {text: 'You make it to Bloodstone Island, but you’re exhausted.', next: 6}, // Removed trailing comma
        {text: 'A sudden storm hits, and you’re thrown off course.', next: 7}, // Added periods
      ],
  ),
  createStoryStep(
      ...
      [
        {text: 'Approach the cave cautiously.', next: 8}, // Added period
        {text: 'Charge into the cave, eager to find the treasure.', next: 9}, // Added period
      ],
  ),
  createStoryStep(
  ...
  createStoryStep(
      'You press on, determined to find the treasure. The cave twists and turns, leading you deeper into the island. ' +
...
function handleOptionSelection(optionIndex) {
  displayStoryNode(story[currentStoryIndex].option[optionIndex - 1].next);
}
